...
# Include the Predicate visual Ft (optional)
...
# DATA_DIR = '/home/nmduy/Graphs/GraSim/LemmaCaptionData'
DATA_IMG_DIR = '/home/nmduy/Graphs/GraSim/OriginalData'
# DATA_IMG_DIR = '/home/nmduy/Graphs/GraSim/MST_ImageData'
...
#word2idx_cap = joblib.load(f"../LemmaCaptionData/flickr30k_caps_word2idx.joblib")
...
subset = 'train'
images_data_train = joblib.load(f"{DATA_IMG_DIR}/flickr30k_{subset}_lowered_images_data.joblib")
caps_data_train = joblib.load(f"{DATA_DIR}/flickr30k_{subset}_lowered_caps_data.joblib") # lowered
...
subset = 'val'
images_data_val = joblib.load(f"{DATA_IMG_DIR}/flickr30k_{subset}_lowered_images_data.joblib")
caps_data_val = joblib.load(f"{DATA_DIR}/flickr30k_{subset}_lowered_caps_data.joblib") # lowered
...
init_embed_model_weight_cap = joblib.load(f'{DATA_DIR}/init_glove_embedding_weight_lowered_train_val.joblib') # lowered
init_embed_model_weight_cap = torch.FloatTensor(init_embed_model_weight_cap)
init_embed_model_weight_img_obj = joblib.load(f'../OriginalData/init_glove_embedding_weight_lowered_img_obj.joblib')
init_embed_model_weight_img_obj = torch.FloatTensor(init_embed_model_weight_img_obj)
init_embed_model_weight_img_pred = joblib.load(f'../OriginalData/init_glove_embedding_weight_lowered_img_pred.joblib')
...
class Trainer():
    def __init__(self, info_dict):
        ...
        self.params += list(self.embed_model_cap.parameters()) 
        self.params += list(self.sent_model.parameters())
        self.params += list(self.rels_model.parameters())
    ...
    def adjust_learning_rate(self, epoch):
        ...
        lr = self.learning_rate * (0.25 ** (epoch // 15)) # 15 epoch update once
    ...
    def train(self):
        ...
        scheduler = ReduceLROnPlateau(self.optimizer, factor = 0.2, patience=4, 
        ...
        for epochID in range (self.numb_epoch):
            ...
            lossTrain = self.train_epoch(loss_matrix, writer, epochID)
    ...
    # ---------- TRAINING 1 EPOCH ---------
    def train_epoch(self, loss_matrix, writer, epochID):
        print(f"Shuffling Training Dataset")
        self.datatrain.create_pairs(seed=1509+epochID)
        ...
        for batchID, batch in enumerate(dataloadertrain):
            ...
            ## [Image] Embed objects and predicates
            #eb_img_p_o = self.embed_model_img_obj(img_p_o) # embedding          
            #eb_img_p_p = self.embed_model_img_pred(img_p_p) # embedding
            ...
            # [Caption] Embed objects and predicates
            # eb_cap_p_o = self.embed_model_cap(cap_p_o)
            # eb_cap_p_p = self.embed_model_cap(cap_p_p)
            
            ## [Caption] Sentence Model
            rnn_eb_pad_cap_p_s = self.sent_model(eb_pad_cap_p_s, cap_p_len_s) # ncap, max sent len, dim

            ## [Caption] Relations Model
            # Create sequence of rels
            #eb_cap_p_rels = torch.zeros(len(cap_p_e), 3, 300).to(device)
            #for idx in range(len(cap_p_e)):
            #    eb_cap_p_rels[idx,0,:] = eb_cap_p_o[cap_p_e[idx,0]]
            #    eb_cap_p_rels[idx,1,:] = eb_cap_p_p[idx]
            #    eb_cap_p_rels[idx,2,:] = eb_cap_p_o[cap_p_e[idx,1]]

            # Concate for batch processing    
            # len_rels = [3 for _ in range(len(cap_p_e))]
            rnn_eb_cap_p_rels, _ = self.rels_model(eb_pad_cap_p_p, cap_p_len_p) # total rels, dim
            ...
            lossvalue = loss_matrix(img_emb, img_p_numb_o, rnn_eb_pad_cap_p_s, cap_p_len_s, 
            ...
            count += 1
            if (batchID+1) % 300 == 0:
                print(f"Batch Idx: {batchID+1} / {len(dataloadertrain)}: Loss Train {loss_report/count}")
    ...
    def encode_image_sgg(self, image_sgg, batch_size=16):
        ...
        return img_obj_emb, img_pred_emb, img_numb_o_all, img_numb_p_all
    ...
    def encode_caption_sgg(self, caption_sgg, batch_size=1):   
        ...
        with torch.no_grad():
            ...
            for batchID, batch in enumerate(c_dtld):
                ...
                rnn_eb_cap_rels, _ = self.rels_model(eb_pad_cap_p_concate, cap_len_p)
        ...
        return cap_sent_emb, cap_rels_emb, cap_len_sent_all, cap_numb_rels_all
    
    # ---------- VALIDATE ---------
    def validate_retrieval(self, image_sgg, caption_sgg):
        print('---------- VALIDATE RETRIEVAL ----------')
        c_dts = CaptionDataset(caption_sgg=caption_sgg, word2idx=word2idx_cap, numb_sample=None)
        c_dtld = make_CaptionDataLoader(c_dts, batch_size=128, num_workers=0)
        
        img_obj_emb, img_pred_emb, img_numb_o_all, img_numb_p_all = self.encode_image_sgg(image_sgg, batch_size=16)
        cap_sent_emb, cap_rels_emb, cap_len_sent_all, cap_numb_rels_all = self.encode_caption_sgg(caption_sgg, batch_size=64)
